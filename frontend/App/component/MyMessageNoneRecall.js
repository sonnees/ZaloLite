...
import { View, Text, Image, TouchableOpacity, Linking, Modal, StyleSheet, TouchableWithoutFeedback, Button } from 'react-native';
import { Video } from 'expo-av';
import { getTime } from '../utils/CalTime';
import { MessageModal } from '../modal/messageModal';
import { GlobalContext } from '../context/GlobalContext';
import { Dimensions } from 'react-native';
const MyMessageNoneRecall = memo(({ item, conversationOpponent, friend }) => {
  ...
  const handleImageLoad = (event) => {
    ...
    const renderContent = () => {
        ...
        if (key.startsWith('MP4|')||key.startsWith('mp4|')) {
    ...
    const renderImageContent = (imageUrl, item, myUserInfo) => (
        <View>
          ...
          >
            ...
            />
            <Text style={{ fontSize: 8, alignSelf: 'flex-start', color: '#888888', marginTop: -15 }}>
    ...
    const renderFileContent = (key, value, item) => {
